...
the decay widths of autoionization processes. Other comparable relativistic programmes
rely on spherical symmetry \cite{Tulkki92,Fritzsche12} and
therefore, the FanoADC Stieltjes approach
necessary for the calculation of \ac{ICD} decay widths.
...
\begin{table}[h]
  ...
  \begin{tabular}{llr|rr|rr|rr|rr|rr}
   \toprule
    & & \multicolumn{3}{c}{Present} & \multicolumn{2}{c}{Koloren\v{c}} & \multicolumn{2}{c}{Kelly} & \multicolumn{2}{c}{Yarzhemsky} & \multicolumn{2}{c}{exp.}\\
...
It is known in the literature \cite{Aaberg82} that
...
The initial state is characterized by $L_i = 0$, $S_i= \frac12$ and has $g$
symmetry under inversion. The
2p$^{-2}$ $^3P$ final state without the electron is characterized by $L_f=1$
and $S_f = 1$ and is an $g$ state. In order to conserve the momenta, the
...
All calculations in this chapter were performed using the
...
for different initial states can be determined as shown in 
table \ref{table:Xe_open_channels}
These results are the basement for the choice of the final state subspace
in the dcay width calculations.
\begin{table}[h]
  \centering
...
the higher orders were needed. In such cases also the error of the evaluated
...
The total relaitivistic decay width obtained with the FanoADC
...
been thoroughly studied \cite{Heinaesmaeki04}, giving rise
...
\begin{figure}[htb]
  ...
  \caption{Shifted $\Gamma(E)$ of the relativistic FanoADC calculation from
           a 4p$_{3/2}$
           initial state with both $\Gamma$ and $E$ given in atomic units.
           The resonance energy is $E_{res}=0$. The smooth curve shows two
...
table \ref{table:xe_auger_comp}. All decay wdiths are in the order of \unit[1]{eV}.
...
by \unit[1.26]{eV}. 
The decay width of this work overestimates the experimental decay width
by \unit[0.27]{eV}. 
...
\unit[10--100]{eV} as estimated \cite{Heinaesmaeki04}. I therefore conclude
that the broad
...
clusters, which the \ac{MCDF} programmes are not able to calculate, because they rely
