...
\begin{align}
 ...
 E_{sec}^\beta &= E_{in} - E_{fin} \label{equation:E_sec}
\end{align}
...
dependency of the final states compared to the initial state in the model of pairs using
...
p$_{3/2}$ and the p$_{3/2}$ ionization energies. The non-relativistic
opening distance would then be
\unit[16.84]{\AA}.

In an experiment one would observe as many secondary electron peaks as
...
\ref{equation:} shows a pure $R^{-6}$ behaviour.
However, this does entail any information about, whether the channel is
open and hence, whether such a process also is observable. Therefore,
...
with a large spin-orbit coupling. In between \unit[12]{\AA} and
